...
class ContextAnalyzer:
    ...
    def extract_achievements_from_text(self, text: str, company: str = "", role: str = "", year: str = "") -> List[Achievement]:
        """Extract achievements from text content."""
        achievements = []
        
        # Common achievement patterns
        patterns = [
            r'(?:increased|improved|grew|scaled|led|managed|built|launched|reduced|optimized)\s+([^.]*?)\s+(?:by|from|to|at|with)\s+([^.]*?)(?:\.|$)',
            r'(?:achieved|reached|attained|delivered|completed|implemented)\s+([^.]*?)(?:\.|$)',
            r'(?:led|managed|oversaw|directed)\s+([^.]*?)(?:team|project|initiative)(?:.*?)(?:resulting in|leading to|which|that)\s+([^.]*?)(?:\.|$)',
        ]
    ...
    def analyze_past_cover_letter(self, content: str, company: str, position: str, date: str, outcome: str = "") -> PastCoverLetter:
    ...
    def generate_strategic_insights(self, job_description: str, achievements: List[Achievement], 
                                  past_cover_letters: List[PastCoverLetter]) -> List[StrategicInsight]:
        ...
        if relevant_achievements:
            insights.append(StrategicInsight(
                ...
                description=f"Highlight {len(relevant_achievements)} relevant achievements with {relevant_achievements[0].impact_level} impact",
                confidence=0.8,
        ...
        if company_cover_letters:
            ...
            if successful_letters:
                insights.append(StrategicInsight(
                    ...
                    description=f"Found {len(successful_letters)} successful applications to {company_name}",
                    confidence=0.9,
    ...
    def _find_relevant_achievements(self, achievements: List[Achievement], job_keywords: List[str]) -> List[Achievement]:
        ...
        relevant_achievements.sort(key=lambda x: (x.impact_level == "high", len(set(x.tags) & set(job_keywords))), reverse=True)
    ...
    def _analyze_tone_preferences(self, past_cover_letters: List[PastCoverLetter], job_description: str) -> Optional[StrategicInsight]:
        ...
        if successful_letters:
            ...
            return StrategicInsight(
                insight_type="tone",
                description=f"Recommended tone: {recommended_tone} (based on {len(successful_letters)} successful applications)",
                confidence=0.7,
