...
KEY_CATEGORIES = [
    "tools", "team_dynamics", "domain_knowledge", "soft_skills", "responsibilities", "outcomes"
]
...
def prompt_user_for_gaps(report: Dict[str, Tuple[str, str]]) -> Dict[str, str]:
    ...
    print(f"{'Requirement':<25} {'Status':<4} Recommendation")
    print("-"*60)
    for req, (status, rec) in report.items():
        print(f"{req:<25} {status:<4} {rec}")
        if status == '❌' or status == '⚠️':
            print(f"[A]dd blurb  [W]rite custom blurb  [S]kip  [F]uture reuse")
