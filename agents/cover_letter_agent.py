...
class CoverLetterAgent:
    ...
    def __init__(self, user_id: Optional[str] = None, data_dir: str = "data"):
        """Initialize the agent with user context or data directory."""
        # Initialize configuration manager
        self.config_manager = get_config_manager(user_id, Path(data_dir))
    ...
    def _load_blurbs(self) -> Dict[str, List[BlurbDict]]:
        ...
        try:
            ...
            logger.error(f"Unexpected error loading blurbs from {blurbs_path}: {e}")
            raise

    def _load_logic(self) -> LogicDict:
        """Load blurb logic using config manager."""
        try:
            return self.config_manager.load_config("blurb_logic")
        except Exception as e:
            logger.error(f"Failed to load logic via config manager: {e}")
            return {}

    def _load_enhancement_log(self) -> List[EnhancementLogEntry]:
        """Load enhancement log from CSV file."""
        log_path = self.data_dir / "enhancement_log.csv"
        if not log_path.exists():
            logger.debug(f"Enhancement log file not found: {log_path}")
            return []
        ...
        try:
            with open(log_path, "r") as f:
                ...
                logger.debug(f"Loaded {len(log_entries)} enhancement log entries")
                return log_entries
        except FileNotFoundError:
            logger.warning(f"Enhancement log file not found: {log_path}")
            return []
        except csv.Error as e:
            logger.error(f"Error parsing enhancement log CSV {log_path}: {e}")
            return []
        except Exception as e:
            logger.error(f"Unexpected error loading enhancement log from {log_path}: {e}")
            return []

    def _save_enhancement_log(self) -> None:
        """Save enhancement log to CSV file."""
        if hasattr(self, "user_context"):
            # Multi-user mode
            try:
                self.user_context.save_enhancement_log(self.enhancement_log)
                logger.debug("Enhancement log saved via user context")
            except Exception as e:
                logger.error(f"Failed to save enhancement log via user context: {e}")
        else:
            # Legacy mode
            log_path = self.data_dir / "enhancement_log.csv"
            if not self.enhancement_log:
                logger.debug("No enhancement log entries to save")
                return
            ...
            try:
                ...
                logger.debug(f"Saved {len(self.enhancement_log)} enhancement log entries to {log_path}")
            except (IOError, OSError) as e:
                logger.error(f"Failed to write enhancement log to {log_path}: {e}")
            except Exception as e:
                logger.error(f"Unexpected error saving enhancement log: {e}")
    ...
    def _load_targeting(self) -> TargetingDict:
        ...
        if not targeting_path.exists():
            logger.debug(f"Targeting config file not found: {targeting_path}")
            return {}
        ...
        try:
            ...
            logger.error(f"Unexpected error loading targeting config from {targeting_path}: {e}")
            return {}

    def _load_config(self) -> ConfigDict:
        """Load agent configuration using config manager."""
        try:
            return self.config_manager.load_config("agent_config")
        except Exception as e:
            logger.error(f"Failed to load config via config manager: {e}")
            return {}

    def _initialize_google_drive(self) -> Optional[Any]:
    ...
    def _initialize_context_analyzer(self) -> Optional[Any]:
    ...
    def _initialize_resume_parser(self) -> Optional[Any]:
    ...
    def _load_resume(self) -> Optional[Any]:
    ...
    def analyze_contextual_data(self, job_description: str) -> ContextualAnalysisDict:
        ...
        if self.context_analyzer:
            ...
            tone_insight = next(
                (insight for insight in analysis["strategic_insights"] if insight.insight_type == "tone"), None
            )
            if tone_insight:
    ...
    def _load_past_cover_letters(self) -> List[CaseStudyDict]:
        ...
        if self.google_drive and self.google_drive.available:
            gd_materials = self.google_drive.get_supporting_materials(self.config.get("google_drive", {}).get("materials", {}))
                ...
                if self.google_drive.download_file(letter["id"], local_path):
                    try:
                        with open(local_path, "r") as f:
                            ...
                            analyzed_letter = self.context_analyzer.analyze_past_cover_letter(content, company, position, date)
    ...
    def generate_enhanced_cover_letter(
        ...
        if contextual_analysis.get("recommended_achievements"):
            cover_letter = self._include_recommended_achievements(
                cover_letter, contextual_analysis["recommended_achievements"]
            )
    ...
    def _extract_company_name(self, text: str) -> str:
        ...
        # 0. Check first line for company name (common pattern)
        if lines:
            first_line = lines[0]
            # Look for capitalized company name at start
            company_match = re.match(r"^([A-Z][a-zA-Z0-9&\s]+)", first_line)
            if company_match:
                company = company_match.group(1).strip()
                # Filter out common job words
                job_words = {"Staff", "Senior", "Product", "Manager", "PM", "Lead", "Director", "VP", "Engineer", "Developer", "Position"}
                if company not in job_words:
                    print(f"[DEBUG] Extracted company name from first line: {company}")
                    return company
    ...
    def _select_top_case_studies(self, job: JobDescription) -> List[Dict[str, Any]]:
    ...
    def _generate_compelling_opening(self, job: JobDescription) -> str:
        ...
        if "product" in role.lower():
            opening = (
                "I've helped early-stage companies find product-market fit, scale revenue, and build products users love. "
            )
            ...
            opening = "I've helped companies scale revenue and user acquisition through data-driven growth strategies. "
            ...
            opening = "I've helped early-stage companies find product-market fit, scale revenue, and raise capital. "
            opening += "As an operator and product leader, I've built software from scratch, worked alongside investors, and led go-to-market strategy. "
            ...
            opening = "I've helped companies achieve their strategic goals through innovative solutions and execution. "
            opening += (
                f"As a leader with experience in {', '.join(themes[:2])}, I'm excited to bring this expertise to {company}—"
            )
            opening += f"helping you {self._get_role_specific_value(job)}."
    ...
    def _enhance_with_strategic_positioning(self, text: str, job: JobDescription) -> str:
        ...
        if "Series A" not in text and "Series C" not in text:
            text = text.replace(
                "At Aurora Solar",
                "At Aurora Solar, I was the founding PM and helped scale the company from Series A to Series C",
            )

        if "valuation" not in text:
            text = text.replace(
                "captured the majority", "captured the majority of the U.S. solar installer market and reach a $4B valuation"
            )
    ...
    def _extract_mission_from_jd(self, job: JobDescription) -> str:
        ...
        for line in lines:
            if (
                ...
                or "our goal" in line.lower()
                or "we aim" in line.lower()
    ...
    def _propose_custom_closer(self, job: JobDescription, mission_text: str) -> str:
        ...
        if "nira" in job.company_name.lower():
            return (
                "I'm inspired by Nira's mission to accelerate renewables by making grid interconnection faster, "
    ...
    def _apply_brevity_improvements(self, cover_letter: str, job: JobDescription) -> str:
        ...
        filler_phrases = ["I believe", "I think", "that said"]
        ...
        # IC role detection
        is_ic_role = (
            "senior" not in job_title_lower
            and "lead" not in job_title_lower
            and "director" not in job_title_lower
            and "vp" not in job_title_lower
            and not is_leadership_role
        )
    ...
    def process_job_description(
        self, job_text: str, debug: bool = False, explain: bool = False, track_enhance: bool = False, interactive: bool = False
    ) -> JobProcessingResult:
        ...
        if llm_enabled and os.getenv("OPENAI_API_KEY"):
            try:
                ...
                try:
                    ...
                    enhancement_result = enhance_with_contextual_llm(jd_text=job_text, cl_text=cover_letter, metadata=metadata)
                    ...
                    if enhancement_result.confidence_score > 0.5:
                        ...
                        logger.warning(
                            f"LLM enhancement confidence too low ({enhancement_result.confidence_score:.2f}), keeping original draft"
                        )
        ...
        if enhancement_result and enhancement_result.confidence_score > 0.5:
            try:
                ...
                comparison_file = draft_agent.save_draft_comparison(
                    original_draft=original_draft, enhanced_draft=cover_letter, enhancement_result=enhancement_result
                )
...
"""
        ...
        if self.google_drive and self.google_drive.available:
            try:
                file_id = self.google_drive.upload_cover_letter_draft(cover_letter, job.company_name, job.job_title, job.score)
                if file_id:
    ...
    def _apply_llm_suggestion(self, cover_letter: str, suggestion: Dict[str, Any]) -> str:
    ...
    def _save_new_blurb(self, suggestion: Dict[str, Any]) -> None:
        ...
        if section not in blurbs:
            blurbs[section] = []
        blurbs[section].append({"id": f"llm_{len(blurbs[section])+1}", "tags": [], "text": suggestion.get("suggestion", "")})
        with open(blurb_db_path, "w") as f:
            yaml.safe_dump(blurbs, f)

    def get_enhancement_suggestions(self, status: Optional[str] = None) -> List[EnhancementLogEntry]:
    ...
    def update_enhancement_status(self, job_id: str, enhancement_type: str, status: str, notes: str = "") -> None:
