...
class ResumeParser:
    ...
    def _identify_sections(self, text: str) -> Dict[str, ResumeSection]:
                ...
                if re.search(pattern, line_lower):
                    ...
                    if current_section and section_content:
                        sections[current_section] = ResumeSection(
                            title=current_section, content="\n".join(section_content), start_line=section_start, end_line=i - 1
                        )
        ...
        if current_section and section_content:
            sections[current_section] = ResumeSection(
                title=current_section, content="\n".join(section_content), start_line=section_start, end_line=len(lines) - 1
            )
    ...
    def _extract_education(self, sections: Dict[str, ResumeSection]) -> List[Education]:
        ...
        for line in lines:
            ...
            if institution_match:
                education.append(
                    Education(institution=institution_match.group(1).strip(), degree="", field="", duration="", location="")
                )
        ...
        for category, skill_list in self.skill_categories.items():
            for skill in skill_list:
                if skill.lower() in text:
                    ...
                    skills.append(Skill(name=skill, category=category, proficiency=proficiency))
